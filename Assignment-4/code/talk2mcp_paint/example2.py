...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    try:
        ...
        paint_window.click_input(coords=(419, 72 ))
        time.sleep(0.2)
        ...
        canvas.press_mouse_input(coords=(x1+1800, y1))
        canvas.move_mouse_input(coords=(x2+1800, y2))
        canvas.release_mouse_input(coords=(x2+1800, y2))
...
async def add_text_in_paint(text: str) -> dict:
    ...
    try:
        ...
        paint_window.click_input(coords=(268, 80))
        time.sleep(0.5)
        ...
        canvas.click_input(coords=(490, 690))
        time.sleep(0.5)
        ...
        canvas.click_input(coords=(495, 685))
...
async def open_paint() -> dict:
    ...
    try:
        ...
        # Get all monitors to find the secondary monitor position
        from win32api import EnumDisplayMonitors
        monitors = EnumDisplayMonitors()

        # Find the secondary monitor (not at 0,0)
        secondary_monitor = None
        for monitor in monitors:
            handle, device, rect = monitor
            x1, y1, x2, y2 = rect
            if x1 != 0 or y1 != 0:  # This is not the primary monitor
                secondary_monitor = (x1, y1, x2, y2)
                break

        if secondary_monitor:
            x1, y1, x2, y2 = secondary_monitor
            # Move window to secondary monitor's top-left corner
            win32gui.SetWindowPos(
                paint_window.handle,
                win32con.HWND_TOP,
                x1 + 10, y1 + 10,  # Small offset from top-left
                0, 0,
                win32con.SWP_NOSIZE
            )
